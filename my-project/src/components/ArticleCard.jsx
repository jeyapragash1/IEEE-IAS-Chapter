    ...
    >
      <div className="group bg-[#111827] rounded-xl border border-gray-800 overflow-hidden transform transition-all duration-300 hover:shadow-2xl hover:shadow-[#00A39C]/20 hover:-translate-y-2">
        ...
        <div className="p-6">
          ...
          <h3 className="text-xl font-bold text-gray-200 mb-3 group-hover:text-[#00A39C] transition-colors h-14">{article.title}</h3>
          <div className="flex flex-wrap gap-2">
            {article.tags.map(tag => (
              <span key={tag} className="text-xs font-semibold bg-gray-700 text-gray-300 px-2 py-1 rounded">{tag}</span>
            ))}
          </div>
        </div>
      </div>
